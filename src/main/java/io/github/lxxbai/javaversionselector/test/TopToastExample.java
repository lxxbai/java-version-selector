...
public class TopToastExample extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        // 创建按钮，点击后显示 Toast
        Button showToastButton = new Button("显示顶部 Toast");
        showToastButton.setOnAction(event -> showCustomToast(root, "这是一条从顶部弹出的 Toast"));
        ...
        primaryStage.setTitle("JavaFX 顶部 Toast 示例");
        primaryStage.show();
    }
    ...
    private void showCustomToast(StackPane root, String message) {
        ...
        StackPane.setMargin(toastLabel, new Insets(20, 0, 0, 0));  // 设置顶部边距

        // 创建滑入动画
        TranslateTransition slideIn = new TranslateTransition(Duration.millis(300), toastLabel);
        slideIn.setFromY(-toastLabel.prefHeight(-1));  // 从顶部上方开始
        slideIn.setToY(0);  // 滑动到当前位置
        slideIn.play();
